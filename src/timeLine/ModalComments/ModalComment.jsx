...
function ModalComment({changeButtonSave, isClickedButtonSave, setClickedHeart, clickedHeart , openShareModal, setListComments, userLoged, listComments, publicationUrl, openOptionsModal, handleClickComment, likes, username, url}) {
    const [activedButtonPost, setActivedButtonPost] = useState(false);
    ...
    const activeBottonPost = ()=>{
        if(inputPost.current.value !== "") {
            setActivedButtonPost(true);
            return
        }
        setActivedButtonPost(false);
    }

    const sendPost = (e)=>{
        e.preventDefault();
        if(inputPost.current.value === "") return;
        
        const newListComments = [...listComments];
        newListComments.push(inputPost.current.value);
                ...
                <div className="contentModal_info">
                    <div className="contentModal_info-header">
                        <div className="contentModal_header-user">
                            <div className="contentModal_header-contentImg">
                    ...
                    <div className="contentModal_info-comments">
                        {listComments.length !== 0 ?             
                            listComments.map((comment, index)=>(
                                userLoged.map((user)=>(
                                    <div key={index} className="contentModal_info-comment">
                                        <img className='contentModal_info-comment-img' src={user.picture.medium} alt="perfil" />
                                        <div className="contentModal_header-user contentModal_info_comment-user">
                                            <div className="contentModal_header-user-pf">
                                                <h4>{user.login.username}</h4>
                                                <span>{comment}</span>      
                                            </div>
                                            <div className="contentModal_header-user-details">
                                                <button>1s</button>
                    ...
                    <div className="contentModal_info-reactions">
                        <div className="publication_reactions">
                            <div className="publication_reactions-left">
                                ...
                                <button className='publication_reactions-icons publication_reactions-comment'>                                
                                ...
                                <button onClick={openShareModal} className='publication_reactions-icons publication_reactions-share'>                                    
                                    <svg aria-label="Compartir publicación" className='icon-hover' color="rgb(0, 0, 0)" fill="rgb(0, 0, 0)" height="24" role="img" viewBox="0 0 24 24" width="24"><title>Compartir publicación</title><line fill="none" stroke="currentColor" stroke-linejoin="round" stroke-width="2" x1="22" x2="9.218" y1="3" y2="10.083"></line><polygon fill="none" points="11.698 20.334 22 3.001 2 3.001 9.218 10.084 11.698 20.334" stroke="currentColor" stroke-linejoin="round" stroke-width="2"></polygon></svg>
                                </button>
                            </div>
                            <button onClick={changeButtonSave} className='publication_reactions-icons'>
                                {isClickedButtonSave ? 
                                    ...
                                    :
                                    <svg aria-label="Guardar" className='icon-hover' color="rgb(0, 0, 0)" fill="rgb(0, 0, 0)" height="24" role="img" viewBox="0 0 24 24" width="24"><title>Guardar</title><polygon fill="none" points="20 21 12 13.44 4 21 4 3 20 3 20 21" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></polygon></svg>
                                }
                            </button>                
                        </div>
                    ...
                    <div className="contentModal_info-post">
                        <form action="" onSubmit={sendPost}>                            
                            ...
                            <input ref={inputPost} onChange={activeBottonPost} className='contentModal_post-input' name='post' placeholder='Add a comment' type="text" />                            
                            <button className='publication_post-buttonPost' style={activedButtonPost ? {opacity:"1", pointerEvents:"all"} : {opacity:".5", pointerEvents:"none"}}>Post</button>
                        </form>
                    </div>
                </div>
            </div>
